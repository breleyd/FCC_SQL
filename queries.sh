...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g JOIN teams t ON t.team_id IN (g.winner_id, g.opponent_id) WHERE g.round = 'Eighth-Final' AND g.year = 2014;")"
...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g JOIN teams t ON t.team_id IN (g.winner_id, g.opponent_id) WHERE t.name LIKE 'Co%' ")"
